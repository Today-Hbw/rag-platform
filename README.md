...
| 6 | 检索评估 eval（metrics/runner/compare/harvest，`python -m eval`） | ✅ 框架（真实 run/golden 待内网/人工） |
| 7 | Docker 分包 + 定时调度 | ⏳ 待做 |
...
顶层 `eval/` 工具包，锚定 `doc_id` 做 @k 指标评估（hit_rate/recall/precision/mrr/ndcg）。

```bash
python -m eval harvest --logs-dir <data_root>/logs --out eval/datasets/candidates.jsonl  # 从日志 pooling 候选
python -m eval run     --dataset eval/datasets/smoke.jsonl --out runs/baseline.json        # 跑评估（需内网）
python -m eval compare --before runs/baseline.json --after runs/tuned.json --fail-on-regress
```

数据集标注与快照约定见 [`eval/datasets/README.md`](eval/datasets/README.md)。
